...
public class RocksDBProvider {
  ...
  public static RocksDB initLevelDB(File dbFile, StoreVersion version, ObjectMapper mapper) throws
      IOException {
    RocksDB tmpDb = null;
    if (dbFile != null) {
      ...
      try {
        ...
        if (e.getStatus().getCode() == Status.Code.NotFound) {
          ...
          // the rocksdb file seems to be corrupt somehow.  Lets just blow it away and create a new
