...
import org.apache.spark.sql.catalyst.expressions.{Alias, Attribute, AttributeSet, Cast, Divide, Expression, ExprId, NamedExpression}
...
object PushPartialAggregationThroughExpand extends Rule[LogicalPlan] {
   ...
   *
   * For functions whose case classes carry semantics-bearing fields (`Sum.evalMode`,
  ...
  private def decompose(ae: AggregateExpression): AggMapping = ae.aggregateFunction match {
    case sum: Sum =>
      ...
      // Preserve evalMode (ANSI / TRY) and the resolved resultDataType so the outer
      // `Sum` keeps the original overflow semantics and output type. Mirrors v4's
      // `PushPartialAggregationThroughJoin.replaceAliasName`.
      val outerFn = Sum(partial.toAttribute, sum.evalMode, Some(sum.dataType))
