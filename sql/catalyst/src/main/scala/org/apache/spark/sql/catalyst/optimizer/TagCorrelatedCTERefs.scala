...
object TagCorrelatedCTERefs extends Rule[LogicalPlan] {
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = {
    // The tag has exactly one consumer, and it is behind this flag: with auto-CTE caching off,
    // `InlineCTE`'s carve-out never runs and `ReplaceCTERefWithCache` returns early, so the
    // walk below fills a field nobody reads. `TagPruningVetoCTE` checks the flag the same way.
