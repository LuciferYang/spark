...
object SQLConf {
  ...
  val PARTIAL_AGGREGATION_OPTIMIZATION_ENABLED =
    buildConf("spark.sql.optimizer.partialAggregationOptimization.enabled")
      .internal()
      .doc("When true, the query optimizer will introduce partial aggregations to " +
        "optimize query. This optimization applies to: " +
  ...
  val SORT_KEY_REORDERING_ENABLED =
    buildConf("spark.sql.sortKeyReordering.enabled")
      .doc("When true, allow the fork-only SortKeyReordering rule to permute the " +
        ...
        "showed a wall-time win at the only scale we tested, SF=30). Enabling " +
        "this also makes the SMJ report a requiredChildDistribution whose key " +
        "ORDER differs from the order the upstream shuffle used -- execution is " +
        "correct because equi-join matches co-locate under either hash, but see " +
        "SortKeyReordering's scaladoc for why this is off by default.")
      .internal()
      .version("4.2.0")
      .withBindingPolicy(ConfigBindingPolicy.SESSION)
      .booleanConf
  ...
  val SORT_KEY_REORDERING_THRESHOLD =
    buildConf("spark.sql.sortKeyReordering.threshold")
      ...
      .withBindingPolicy(ConfigBindingPolicy.SESSION)
      .doubleConf
      .checkValue(_ >= 1.0, "The reordering threshold must be at least 1.0, since a " +
        "ratio below 1.0 is impossible: the best permutation's cost is by definition " +
        "no greater than the original's.")
