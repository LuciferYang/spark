 ...
 * massaging). Closes the §4.2 "push gamma below project" gap described in
 ...
 * `Alias(a + 1, "x")`) are not handled in this iteration -- Synapse §4.2
...
object PushPartialAggregationThroughProject extends Rule[LogicalPlan] {
  ...
  private def eligible(
    ...
    // derived) are rejected; a follow-up can handle the §4.2 "alias used only
