...
import org.apache.spark.sql.catalyst.plans.logical.{Aggregate, AggregateBase, FinalAggregate, Join, LogicalPlan, PartialAggregate, Project}
import org.apache.spark.sql.catalyst.rules.Rule
import org.apache.spark.sql.catalyst.trees.TreePattern.{AGGREGATE, JOIN, PROJECT, UNION, WINDOW}
 ...
 *  7. A `Join` or `Union` sits somewhere below the Project (O(1) cached
 *     tree-pattern test), so the partial has something to compress against
 *     instead of being stranded above a leaf scan (reuse-breaking; see q9).
 *
 ...
 *
 * Idempotency: the rule matches only `Aggregate(_, _, Project, _)` and rewrites
 * the outer node to a [[FinalAggregate]], so its own output -- and the
 * [[FinalAggregate]] output of the earlier sibling
 * [[PushPartialAggregationThroughJoin]] in this `Once` batch -- is never a fresh
 * match, so there is no double aggregation.
 *
...
object PushPartialAggregationThroughProject extends Rule[LogicalPlan] {
  ...
  override def apply(plan: LogicalPlan): LogicalPlan = {
    // Skip the whole plan when it contains any Window: in window queries the inner
    // aggregate is often shared by a self-join feeding the Window (TPC-DS q47/q57),
    // and pushing a distinct partial aggregate into it breaks that reuse. The veto
    // must be whole-plan (not the per-subtree prune predicate) because the Window
    // sits above the matched aggregate, so a per-subtree check would miss it.
    if (!conf.partialAggregationOptimizationEnabled || plan.containsPattern(WINDOW)) {
      plan
    } else {
  ...
  private def eligible(
    ...
    // Only fire when a Join or Union sits anywhere below the Project (an O(1) cached
    // tree-pattern test) for the partial to compress against. Landing it above a leaf
    // scan yields no benefit and can break reuse of a shared sub-tree (TPC-DS q9: 15
    // scalar sub-queries stop sharing one scan, +20 exchanges). Expand is excluded:
    // the Expand rule runs earlier in this batch, so a Project over an Expand has no
    // further push target and would only no-op-decompose.
    if (!project.child.containsAnyPattern(JOIN, UNION)) {
      return None
    }
    // Don't re-fire on the sibling PushPartialAggregationThroughJoin's output (it runs
    // earlier in this `Once` batch): `Aggregate(Project(Join(PartialAggregate, ...)))`
    // already carries pushed partials, so re-firing would stack a redundant
    // partial/final layer. Mirrors the Join rule's own `children.exists(AggregateBase)`
    // idempotency guard.
    project.child match {
      case j: Join if j.children.exists(_.isInstanceOf[AggregateBase]) => return None
      case _ =>
    }
