...
import org.apache.spark.sql.catalyst.expressions.{Alias, Attribute, Cast, Divide, EvalMode, Expression}
  ...
  def decompose(ae: AggregateExpression): AggMapping =
    ae.aggregateFunction match {
      case sum: Sum =>
        ...
        // v4: `Sum` takes `evalMode: EvalMode.Value` directly (no NumericEvalContext).
      ...
      case avg: Average =>
        // v4: `Sum` takes `EvalMode.Value` directly (no NumericEvalContext indirection).
