...
import org.apache.spark.sql.catalyst.expressions.{Add, Alias, Ascending, Attribute, AttributeMap, AttributeReference, Cast, CurrentRow, Divide, EqualTo, EvalMode, GreaterThan, Literal, NumericEvalContext, Rand, RowFrame, RowNumber, SortOrder, SpecifiedWindowFrame, Substring, UnboundedPreceding, WindowExpression, WindowSpecDefinition}
...
import org.apache.spark.sql.catalyst.plans.logical.{Aggregate, AggregateHint, ColumnStat, Expand, FinalAggregate, Join, JoinHint, LocalRelation, LogicalPlan, PartialAggregate, Project, Union, Window}
...
class PushPartialAggregationThroughProjectSuite extends StatsEstimationTestBase with PlanTest {
  ...
  private def relation: (LogicalPlan, AttributeReference, AttributeReference,
    ...
    val leaf = StatsTestPlan(
      outputList = Seq(k, v, w, extra),
      rowCount = 1000,
      attributeStats = stats,
      size = Some(1000 * 16))
    // The rule fires only when a heavier operator sits below the Project; wrap the
    // leaf in a Union with an empty same-schema leg (see `heavyChild`).
    (heavyChild(leaf), k, v, w, extra)
  }

  /** Wraps a stats-bearing leaf in a Union with an empty same-schema leg so the
   *  rule's heavy-operator-below eligibility guard is met while row count and column
   *  stats (hence the cost gate) are preserved via CBO Union-stats propagation. */
  private def heavyChild(leaf: StatsTestPlan): Union = {
    val emptyOut = leaf.output.map(_.newInstance())
    val emptyStats = AttributeMap(emptyOut.map(a =>
      a -> ColumnStat(distinctCount = Some(0), nullCount = Some(0))))
    Union(Seq(leaf, StatsTestPlan(emptyOut, 0, emptyStats, size = Some(0))))
  }
  ...
  test("Decimal Avg: excluded (matches Expand/Union/Filter rules)") {
    withRuleEnabled {
      ...
      // Wrap in `heavyChild` so the no-op is attributable solely to the Decimal-Avg
      // exclusion, not to the heavy-operator-below eligibility guard.
      val rel = heavyChild(StatsTestPlan(
        outputList = Seq(dk, dv),
        rowCount = 1000,
        attributeStats = AttributeMap(Seq(
          dk -> ColumnStat(distinctCount = Some(10), nullCount = Some(0)),
          dv -> ColumnStat(distinctCount = Some(500), nullCount = Some(0)))),
        size = Some(1000 * 16)))
      val agg = Aggregate(
  ...
  test("Stats missing: cost gate falls back to ratio=1.0") {
    val k = AttributeReference("k", IntegerType)()
    val v = AttributeReference("v", IntegerType)()
    // No column stats (LocalRelation) so the cost gate falls back to ratio 1.0; the
    // Union satisfies the heavy-operator-below eligibility guard.
    val rel = Union(Seq(LocalRelation(k, v),
      LocalRelation(AttributeReference("k", IntegerType)(),
  ...
  test("Extras-dropped: PartialAggregate aggregateExpressions size locked down") {
    ...
    withRuleEnabled {
      ...
      assert(partial.aggregateExpressions.size == 2,
        s"Expected partial.aggregateExpressions.size == 2; " +
          s"got ${partial.aggregateExpressions.size} in:\n$partial")
    }
  }
  ...
  test("Average.evalMode propagated into partial Sum's evalContext") {
    // `decompose` constructs the partial Sum with
    // `NumericEvalContext(avg.evalMode)`. The existing Avg lock-in test pins
    // the outer Divide's EvalMode.LEGACY; this test pins the *inner* partial
    // Sum's evalMode propagation -- a regression that hard-coded
    // `NumericEvalContext.fromSQLConf` on the partial would slip past every
    // other Avg test.
    withRuleEnabled {
      val (rel, k, v, _, _) = relation
      ...
      assert(partialSums.forall(_.evalContext == NumericEvalContext(EvalMode.ANSI)),
        s"Partial Sum's evalContext must be NumericEvalContext(ANSI); " +
          s"got ${partialSums.map(_.evalContext)}")
    }
  }
  ...
  // --- Tests: eligibility guards (heavy-operator-below, window veto) ---------

  test("Guard: rule does not fire when the Project sits directly over a leaf scan") {
    withRuleEnabled {
      val k = AttributeReference("k", IntegerType)()
      val v = AttributeReference("v", IntegerType)()
      val leaf = StatsTestPlan(Seq(k, v), 1000,
        AttributeMap(Seq(
          k -> ColumnStat(distinctCount = Some(10), nullCount = Some(0)),
          v -> ColumnStat(distinctCount = Some(500), nullCount = Some(0)))),
        size = Some(1000 * 16))
      val agg = Aggregate(Seq(k),
        Seq(k, Alias(AggregateExpression(Sum(v), Complete, isDistinct = false), "sum_v")()),
        Project(Seq(k, v), leaf))
      assert(!hasPreAgg(Optimize.execute(agg)),
        "Rule must not fire with no Join/Union below the Project (q9 reuse guard)")
    }
  }

  test("Guard: rule does not fire when only an Expand (no Join/Union) is below") {
    withRuleEnabled {
      val k = AttributeReference("k", IntegerType)()
      val v = AttributeReference("v", IntegerType)()
      val leaf = StatsTestPlan(Seq(k, v), 1000,
      ...
      val expand = Expand(Seq(Seq(k, v, Literal(0L))), Seq(k, v, gid), leaf)
      val agg = Aggregate(Seq(k),
        Seq(k, Alias(AggregateExpression(Sum(v), Complete, isDistinct = false), "sum_v")()),
        Project(Seq(k, v), expand))
      assert(!hasPreAgg(Optimize.execute(agg)),
        "Rule must not fire when only an Expand (no Join/Union) sits below the Project")
    }
  }

  test("Guard: rule fires when a Join (raw relations) sits below the Project") {
    withRuleEnabled {
      val k = AttributeReference("k", IntegerType)()
      val v = AttributeReference("v", IntegerType)()
      val dk = AttributeReference("dk", IntegerType)()
      val left = StatsTestPlan(Seq(k, v), 1000,
        AttributeMap(Seq(
          k -> ColumnStat(distinctCount = Some(10), nullCount = Some(0)),
          v -> ColumnStat(distinctCount = Some(500), nullCount = Some(0)))),
        size = Some(1000 * 16))
      val dim = StatsTestPlan(Seq(dk), 10,
        AttributeMap(Seq(dk -> ColumnStat(distinctCount = Some(10), nullCount = Some(0)))),
        size = Some(10 * 8))
      val join = Join(left, dim, Inner, Some(EqualTo(k, dk)), JoinHint.NONE)
      val agg = Aggregate(Seq(k),
        Seq(k, Alias(AggregateExpression(Sum(v), Complete, isDistinct = false), "sum_v")()),
        Project(Seq(k, v), join))
      assert(hasPreAgg(Optimize.execute(agg)),
        "Rule must fire when a Join (raw relations) sits below the Project")
    }
  }

  test("Guard: rule does not re-fire on the Join rule's output (Join with pushed partials)") {
    withRuleEnabled {
      val k = AttributeReference("k", IntegerType)()
      val v = AttributeReference("v", IntegerType)()
      val dk = AttributeReference("dk", IntegerType)()
      ...
      val dim = StatsTestPlan(Seq(dk), 10,
        AttributeMap(Seq(dk -> ColumnStat(distinctCount = Some(10), nullCount = Some(0)))),
        size = Some(10 * 8))
      // Mimic PushPartialAggregationThroughJoin's output: a Join whose side is already a
      // PartialAggregate. The Project rule must not stack a second partial/final layer.
      val join = Join(PartialAggregate(Seq(k), Seq(k, v), left), dim, Inner,
        Some(EqualTo(k, dk)), JoinHint.NONE)
      val agg = Aggregate(Seq(k),
        Seq(k, Alias(AggregateExpression(Sum(v), Complete, isDistinct = false), "sum_v")()),
        Project(Seq(k, v), join))
      val optimized = Optimize.execute(agg)
      assert(optimized.isInstanceOf[Aggregate],
        s"Rule must not re-fire on a Join that already carries pushed partials; outer " +
          s"should stay Aggregate, got ${optimized.getClass.getSimpleName}")
    }
  }

  test("Guard: rule skips the whole plan when it contains a Window") {
    withRuleEnabled {
      val (rel, k, v, _, _) = relation
      val agg = Aggregate(Seq(k),
        Seq(k, Alias(AggregateExpression(Sum(v), Complete, isDistinct = false), "sum_v")()),
        Project(Seq(k, v), rel))
      // Sanity: the same plan rewrites when no Window is present.
      assert(hasPreAgg(Optimize.execute(agg)))
      // A Window anywhere in the plan triggers the whole-plan veto (q47/q57 reuse guard).
      val winSpec = WindowSpecDefinition(Seq(k), Seq(SortOrder(k, Ascending)),
        SpecifiedWindowFrame(RowFrame, UnboundedPreceding, CurrentRow))
      val windowed = Window(
        Seq(Alias(WindowExpression(RowNumber(), winSpec), "rn")()),
        Seq(k), Seq(SortOrder(k, Ascending)), agg)
      assert(!hasPreAgg(Optimize.execute(windowed)),
        "Rule must skip the entire plan when a Window is present")
    }
  }
}
