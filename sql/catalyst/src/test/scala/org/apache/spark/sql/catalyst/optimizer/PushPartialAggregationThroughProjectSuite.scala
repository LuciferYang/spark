...
import org.apache.spark.sql.catalyst.expressions.{Add, Alias, Ascending, Attribute, AttributeMap, AttributeReference, Cast, CurrentRow, Divide, EqualTo, EvalMode, GreaterThan, Literal, Rand, RowFrame, RowNumber, SortOrder, SpecifiedWindowFrame, Substring, UnboundedPreceding, WindowExpression, WindowSpecDefinition}
...
class PushPartialAggregationThroughProjectSuite extends StatsEstimationTestBase with PlanTest {
  ...
  test("Average.evalMode propagated into partial Sum's evalMode") {
    ...
    // propagation -- a regression that hard-coded `EvalMode.fromSQLConf` on the
    // partial would slip past every other Avg test.
    withRuleEnabled {
      val (rel, k, v, _, _) = relation
      ...
      assert(partialSums.forall(_.evalMode == EvalMode.ANSI),
        s"Partial Sum's evalMode must be ANSI; " +
