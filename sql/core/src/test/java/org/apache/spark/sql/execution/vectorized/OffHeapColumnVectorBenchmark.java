 ...
 * <ul>
 *   <li>{@code putNulls(rowId, count)}       — off-heap byte fill</li>
 *   <li>{@code putNotNulls(rowId, count)}     — off-heap byte clear</li>
 *   <li>{@code putBooleans(rowId, count, v)}  — off-heap byte fill</li>
 *   <li>{@code putBytes(rowId, count, v)}     — off-heap byte fill</li>
 *   <li>{@code putShorts(rowId, count, v)}    — off-heap short fill</li>
 *   <li>{@code putInts(rowId, count, v)}      — off-heap int fill</li>
 *   <li>{@code putLongs(rowId, count, v)}     — off-heap long fill</li>
 * </ul>
 *
 * <p>To run:
 * <pre>{@code
...
public class OffHeapColumnVectorBenchmark {
    ...
    public void putShorts(BaseState state) {
        state.shortVector.putShorts(0, state.batchSize, (short) 1234);
    }
    ...
    public void putInts(BaseState state) {
        state.intVector.putInts(0, state.batchSize, 123456);
    }
    ...
    public void putLongs(BaseState state) {
        state.longVector.putLongs(0, state.batchSize, 123456789L);
    }
    ...
    public void putFloats(BaseState state) {
        state.floatVector.putFloats(0, state.batchSize, 1234.5f);
    }
    ...
    public void putDoubles(BaseState state) {
      state.doubleVector.putDoubles(0, state.batchSize, 123456789.0);
    }
    ...
    public void putFloats_old(BaseState state) {
      state.oldFloatVector.putFloats(0, state.batchSize, 1234.5f);
    }
    ...
    public void putDoubles_old(BaseState state) {
      state.oldDoubleVector.putDoubles(0, state.batchSize, 123456789.0);
    }
