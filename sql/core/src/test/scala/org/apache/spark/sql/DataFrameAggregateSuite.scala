  ...
  test("test CombineJoinedAggregates") {
    ...
    Seq(false, true).foreach { enabled =>
      withSQLConf(SQLConf.COMBINE_JOINED_AGGREGATES_ENABLED.key -> enabled.toString) {
        ...
        val join22 =
          df.where($"date" === 20151123).agg(
            Column(sumWithFilter).as("sum_temp")).join(
            df.where($"date" === 20151124).agg(avg($"temp").as("avg_temp")))
        ...
        val join23 =
          df.where($"date" === 20151123).agg(sum($"temp").as("sum_temp")).join(
            df.where($"date" === 20151124).agg(Column(avgWithFilter).as("avg_temp")))
        checkAnswer(join23, Row(84.0, 18.25))

        val join24 =
          df.where($"date" === 20151123).agg(Column(sumWithFilter).as("sum_temp")).join(
            df.where($"date" === 20151124).agg(Column(avgWithFilter).as("avg_temp")))
        ...
        val join25 =
          df.where($"date" === 20151123).agg(Column(sumWithFilter).as("sum_temp")).join(
            df.where($"date" === 20151124).agg(Column(avgWithFilter).as("avg_temp"))).join(
            df.where($"date" === 20151125).agg(Column(countWithFilter).as("count_temp")))
