...
object ExpandBenchmark extends SqlBasedBenchmark {
  ...
  override def runBenchmarkSuite(mainArgs: Array[String]): Unit = {
    ...
    runBenchmark("Expand: expression-based distinct (col_i + col_j)") {
      // For COUNT(DISTINCT col1 + col2), the pre-aggregate groups by leaf
      // attributes (col1, col2) not the expression. We compare:
      //
      // Low-cardinality leaves (coprime moduli 7, 11, 13, 17, 19, 23):
      //   Each pair has modest unique combinations vs ~10K rows/key -> good dedup
      //
      // High-cardinality leaves (prime moduli 503, 701, 509, 607, 709, 601):
      //   Each pair has ~10K+ unique combinations = row count -> no dedup
      //
      // We vary the number of expression-based distinct aggs (2, 4, 6)
      // to show how the Expand ratio amplifies the effect.
      prepareTable("expand_expr_low", N, keyMod = 1000,
        colMods = Seq(7, 11, 13, 17, 19, 23))
      prepareTable("expand_expr_high", N, keyMod = 1000,
        colMods = Seq(503, 701, 509, 607, 709, 601))

      // Helper to build expression-based distinct queries
      // e.g. numExprDistinct=2 -> COUNT(DISTINCT col1+col2), COUNT(DISTINCT col3+col4)
      def exprDistinctQuery(table: String, numExprDistinct: Int): String = {
        val distinctCols = (0 until numExprDistinct).map { i =>
          val c1 = i * 2 + 1
          val c2 = i * 2 + 2
          s"count(distinct col$c1 + col$c2) as cd${i + 1}"
        }.mkString(",\n  ")
        s"""SELECT key,
           |  $distinctCols
           |FROM $table
      ...
      for (numAggs <- Seq(2, 3)) {
        val sqlLow = exprDistinctQuery("expand_expr_low", numAggs)
        val sqlHigh = exprDistinctQuery("expand_expr_high", numAggs)

        val bench = new Benchmark(
          s"$numAggs expression-based distinct aggs (ratio $numAggs)",
          N, output = output)

        bench.addCase(
            "low-card leaves - baseline", numIters = 5) { _ =>
          withSQLConf(OPTIMIZE_EXPAND_RATIO -> "-1") {
            spark.sql(sqlLow).noop()
          }
        }
        bench.addCase(
            "low-card leaves - optimized", numIters = 5) { _ =>
          withSQLConf(OPTIMIZE_EXPAND_RATIO -> "2") {
            spark.sql(sqlLow).noop()
          }
        }
        bench.addCase(
            "high-card leaves - baseline", numIters = 5) { _ =>
          withSQLConf(OPTIMIZE_EXPAND_RATIO -> "-1") {
            spark.sql(sqlHigh).noop()
          }
        }
        bench.addCase(
            "high-card leaves - optimized", numIters = 5) { _ =>
          withSQLConf(OPTIMIZE_EXPAND_RATIO -> "2") {
            spark.sql(sqlHigh).noop()
          }
        }
