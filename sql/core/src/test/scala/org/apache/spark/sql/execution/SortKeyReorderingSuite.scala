...
class SortKeyReorderingSuite extends QueryTest with SharedSparkSession {
  ...
  override protected def sparkConf: org.apache.spark.SparkConf =
    super.sparkConf
      ...
      // Explicitly load the extension. Production uses ServiceLoader via the
      // META-INF/services file; the test framework's SharedSparkSession can be
      // flaky about picking it up across test-classes, so set the conf directly
      // to make the wiring deterministic.
      .set("spark.sql.extensions",
        classOf[SortKeyReorderingExtension].getName)
  ...
  test("wiring: SortKeyReordering is registered as a queryStagePrepRule") {
    val prepRuleClasses = spark.sessionState.adaptiveRulesHolder.queryStagePrepRules
      .map(_.getClass.getSimpleName)
    assert(prepRuleClasses.contains(classOf[SortKeyReordering].getSimpleName),
      s"expected SortKeyReordering in queryStagePrepRules, got: $prepRuleClasses")
  }
