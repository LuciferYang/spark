...
class JDBCSuite extends QueryTest with SharedSparkSession {
  ...
  test("SELECT * WHERE (simple predicates)") {
    ...
    assert(checkNotPushdown(sql("SELECT * FROM foobar WHERE (THEID + 1) < 2")).collect().length == 0)
    assert(checkNotPushdown(sql("SELECT * FROM foobar WHERE (THEID + 2) != 4")).collect().length == 2)
  }
  ...
  test("SELECT * WHERE (quoted strings)") {
    assert(sql("select * from foobar").where($"NAME" === "joe 'foo' \"bar\"").collect().length === 1)
  }
