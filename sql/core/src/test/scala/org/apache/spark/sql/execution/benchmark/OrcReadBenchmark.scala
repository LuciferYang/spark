...
object OrcReadBenchmark extends SqlBasedBenchmark {
  ...
  def filterFromWideTableBenchmark(values: Int, width: Int, files: Int): Unit = {
    ...
    withTempPath { dir =>
      withTempTable("t1", "orcTable") {
        ...
        prepareTable(dir, spark.sql("SELECT * FROM t1").repartition(files))

        benchmark.addCase("SQL ORC Vectorized") { _ =>
          spark.sql(s"SELECT c$middle FROM orcTable where c$middle = $middle").noop()
        }
