...
import org.apache.spark.sql.execution.datasources.parquet.{VectorizedPlainValuesReader, VectorizedSingleBufferPlainValuesReader}
...
class ColumnarBatchSuite extends SparkFunSuite {
  ...
  testVector("Boolean APIs with VectorizedSingleBufferPlainValuesReader", 1024, BooleanType) {
    column =>
      val reference = mutable.ArrayBuffer.empty[Boolean]
      ...
      implicit def intToByte(i: Int): Byte = i.toByte
      val buf = ByteBuffer.wrap(Array(0x33, 0x5A, 0xA5, 0xCC, 0x0F, 0xF0, 0xEE, 0x77, 0x88))
      val reader = new VectorizedSingleBufferPlainValuesReader()
      reader.initFromPage(0, ByteBufferInputStream.wrap(buf))
