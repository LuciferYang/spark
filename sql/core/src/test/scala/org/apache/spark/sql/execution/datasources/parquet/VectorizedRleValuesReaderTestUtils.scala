...
/**
 * Shared helpers for tests and benchmarks that exercise `VectorizedRleValuesReader` directly.
 */
private[parquet] object VectorizedRleValuesReaderTestUtils {

  /** Encodes `values` using parquet-mr's RLE/bit-packing hybrid encoder (no length prefix). */
  ...
  /**
   * Produces little-endian plain INT32 bytes for `count` sequential integers following `f(i)`.
   * Matches the layout expected by `VectorizedPlainValuesReader` (see its `readIntegers` impl
   * which calls `putIntsLittleEndian`).
   */
  ...
  /**
   * Stateless updater that reads INT32 values. Equivalent to the package-private
   * `ParquetVectorUpdaterFactory.IntegerUpdater`; duplicated here because `IntegerUpdater` is
   * not accessible from test classloaders under `spark-submit --jars`.
   */
