...
import org.apache.spark.sql.catalyst.plans.logical.{PartialAggregate, RepartitionByExpression}
...
class AutoCTECacheCorrectnessSuite extends QueryTest with SharedSparkSession {
  ...
  test("SPARK-40193: the merged-subquery CTE def itself is NOT auto-cached") {
    prepareData()
    // `MergeSubplans` runs in the batch right before the one hosting `ReplaceCTERefWithCache`
    // and synthesises a `CTERelationDef` with `underSubquery = true` to host the merged scalar
    // subquery. Execution-time subquery reuse already evaluates that def once -- which is why
    // `ReplaceCTERefWithRepartition` deliberately skips adding a shuffle for it -- so
    // materialising it as an `InMemoryRelation` swaps a cheap mechanism for an expensive one
    // (cache entry, accumulators, storage blocks, cross-query TTL) around what is typically a
    // one-row aggregate. Both tagging batches run BEFORE `MergeSubplans`, so such a def reaches
    // `shouldAutoCache` with `correlatedSubqueryRef = false` and `pruningVeto = false` purely
    // because nothing was in a position to tag it.
    //
    // No user-written CTE here, so the ONLY def in the plan is the synthetic one: any cache
    // entry at all means the `underSubquery` gate regressed.
    withSQLConf(SQLConf.AUTO_REUSED_CTE_ENABLED.key -> "true") {
      val sql =
        """SELECT
          |  (SELECT avg(value) FROM auto_cte_corr_test),
          |  (SELECT sum(value) FROM auto_cte_corr_test),
          |  (SELECT count(value) FROM auto_cte_corr_test)""".stripMargin

      val rows = spark.sql(sql).collect()
      assert(rows.length == 1, "the probe query must return exactly one row")
      assert(cachedEntries == 0,
        "a merged-subquery CTE def (underSubquery = true) must be left to " +
        "subquery reuse, not materialised as an InMemoryRelation")
      assert(countInMemoryRelations(sql) == 0,
        "no InMemoryRelation should appear for a merged scalar subquery")
    }
  }
  ...
  test("sync: InlineCTE carve-out and AutoCTECache eligibility agree (structural gate)") {
    ...
    withSQLConf(
      ...
      val eligibleCases = Seq(
        ...
        // An Aggregate above the join is required: a body whose joins are not
        // dominated by a collapsing operator is row-expanding, and
        // `isRowExpanding` declines those on both sides. See
        // `ReplaceCTERefWithCache.isRowExpanding`.
        "WITH c AS (SELECT t1.id k, count(*) v FROM range(20) t1 " +
          "JOIN range(20) t2 ON t1.id = t2.id GROUP BY t1.id) " +
      ...
      // Negative case: a row-expanding body (a Join with no collapsing operator
      // above it) must NOT cache, and both sides must agree so the def never
      // reaches `ReplaceCTERefWithRepartition`. This is the shape of TPC-DS q95's
      // `ws_wh` body, whose caching measured 15.3x SLOWER than the inlined
      // baseline at 100TB. Both predicates are purely structural, which is what
      // lets them agree despite running at very different points in the optimizer.
      spark.sharedState.autoCTECacheManager.clearAll(spark)
      spark.sql(
        """WITH c AS (SELECT t1.id k FROM range(20) t1
          |  JOIN range(20) t2 ON t1.id = t2.id AND t1.id <> t2.id)
          |SELECT a.k FROM c a JOIN c b ON a.k = b.k""".stripMargin).collect()
      assert(cachedEntries == 0,
        "A row-expanding CTE body (join with no aggregate above it) must NOT " +
        "cache; see ReplaceCTERefWithCache.isRowExpanding.")
    }
  }

  test("sync: the gates agree across the partial-aggregation rewrite") {
    prepareData()
    // `Aggregate`, `PartialAggregate` and `FinalAggregate` are SIBLINGS under
    // `AggregateBase` with no subtyping between them, and the batch
    // "Partial Aggregation Optimization" sits BETWEEN the two gates: after
    // `Batch("Inline CTE")` and before `Batch("Replace CTE with Repartition")`. With
    // `partialAggregationOptimization.enabled` on it rewrites an `Aggregate` over a join into
    // `FinalAggregate` over `PartialAggregate` (`PushPartialAggregationThroughJoin`), so a gate
    // matching `Aggregate` alone sees the aggregate on the InlineCTE side and NOT on the cache
    // side.
    //
    // That is the asymmetry both gates exist to prevent: InlineCTE keeps the def,
    // `ReplaceCTERefWithCache` declines it as row-expanding, and it lands on
    // `ReplaceCTERefWithRepartition` -- 22 min against 1.4 min on TPC-DS q95. It was a live bug
    // on the 3.5.5 fork: with this conf on, q23a's cache entries dropped 4 -> 2 and two
    // `RepartitionByExpression` appeared; q23b 6 -> 2 with four. Both gates match
    // `AggregateBase` for exactly this reason. Note the cluster runs with this conf ON while the
    // `SQLConf` default is off, which is why the bug survived a whole review round there.
    Seq("false", "true").foreach { pushPartialAgg =>
      withSQLConf(
        SQLConf.AUTO_REUSED_CTE_ENABLED.key -> "true",
        SQLConf.AUTO_CTE_CACHE_MIN_SIZE_BYTES.key -> "0",
        SQLConf.PARTIAL_AGGREGATION_OPTIMIZATION_ENABLED.key -> pushPartialAgg,
        // benefitRatio=1.0, not the 0.3 default: `PushPartialAggregationThroughJoin` has its own
        // cost gate and at 0.3 it declines to rewrite this 20-row body, which would make the
        // test VACUOUS -- it would pass with either predicate because no `FinalAggregate` is
        // ever produced. The real TPC-DS queries hit this at the default ratio because their
        // bodies are large enough to clear the cost gate.
        SQLConf.PARTIAL_AGGREGATION_OPTIMIZATION_BENEFIT_RATIO.key -> "1.0") {
        // A body shaped like the TPC-DS bodies that regressed: a join with an aggregate above
        // it, which is what the rewrite rewrites.
        val cteSql =
          """WITH c AS (SELECT t1.id k, count(*) v FROM range(20) t1
            |  JOIN range(20) t2 ON t1.id = t2.id GROUP BY t1.id)
            |SELECT a.v + b.v FROM c a JOIN c b ON a.k = b.k""".stripMargin

        // Pin the fixture BEFORE asserting anything about the gates. Unless the rewrite really
        // fires, the body still carries a plain `Aggregate` at both gates, `Aggregate` and
        // `AggregateBase` behave identically, and every assertion below passes no matter which
        // one the gates match -- the test would be vacuous exactly where it is supposed to bite.
        // Read with the feature OFF because a cached body hides inside an `InMemoryRelation`,
        // which `collectWithSubqueries` does not descend into.
        withSQLConf(SQLConf.AUTO_REUSED_CTE_ENABLED.key -> "false") {
          val bare = spark.sql(cteSql).queryExecution.optimizedPlan
          val partials = bare.collectWithSubqueries { case a: PartialAggregate => a }.size
          assert((partials > 0) == (pushPartialAgg == "true"),
            s"fixture no longer matches its purpose: with " +
            s"partialAggregationOptimization=$pushPartialAgg the plan has $partials " +
            s"PartialAggregate nodes. The rewrite must fire when the conf is on and not when " +
            s"it is off, or this test cannot tell `Aggregate` from `AggregateBase`:\n$bare")
        }
        ...
        df.collect()
        assert(cachedEntries >= 1,
          s"a join-with-aggregate body must cache with " +
          s"partialAggregationOptimization=$pushPartialAgg. Losing it here means the two gates " +
          s"disagree across the PartialAggregate/FinalAggregate rewrite, which drops the def " +
          s"onto ReplaceCTERefWithRepartition.")
        val plan = df.queryExecution.optimizedPlan
        assert(plan.collectWithSubqueries { case r: RepartitionByExpression => r }.isEmpty,
          s"the def must not fall through to the repartition shuffle " +
          s"(partialAggregationOptimization=$pushPartialAgg):\n$plan")
        assert(plan.resolved, s"plan must be resolved:\n$plan")

        // The row-expanding rejection must ALSO survive the rewrite: no aggregate above the
        // join means neither gate may cache it, whichever node classes the rewrite produced.
        spark.sharedState.autoCTECacheManager.clearAll(spark)
        spark.sql(
          """WITH c AS (SELECT t1.id k FROM range(20) t1
            |  JOIN range(20) t2 ON t1.id = t2.id AND t1.id <> t2.id)
            |SELECT a.k FROM c a JOIN c b ON a.k = b.k""".stripMargin).collect()
        assert(cachedEntries == 0,
          s"a row-expanding body must stay declined with " +
          s"partialAggregationOptimization=$pushPartialAgg")

        // Second shape, aimed at the OTHER cache-side use of `AggregateBase`. The body above
        // contains a Join, so `isStructurallyExpensive` answers true on its `case _: Join` arm
        // no matter what its aggregate arm matches -- only `isRowExpanding.collapsesRows` is
        // load-bearing there. A body whose aggregate sits over a UNION has no Join, so the
        // aggregate arm is the only thing that can make it structurally expensive, and
        // `PushPartialAggregationThroughUnion` rewrites it into `FinalAggregate` over
        // `PartialAggregate` just the same. Narrowing that arm to `Aggregate` makes this rule
        // decline a def `InlineCTE` kept, which is the asymmetry under test.
        val unionCteSql =
          """WITH c AS (SELECT k, count(*) v FROM
            |  (SELECT id k FROM range(20) UNION ALL SELECT id k FROM range(20)) u
            |  GROUP BY k)
            |SELECT a.v + b.v FROM c a JOIN c b ON a.k = b.k""".stripMargin
        withSQLConf(SQLConf.AUTO_REUSED_CTE_ENABLED.key -> "false") {
          val bare = spark.sql(unionCteSql).queryExecution.optimizedPlan
          val partials = bare.collectWithSubqueries { case a: PartialAggregate => a }.size
          assert((partials > 0) == (pushPartialAgg == "true"),
            s"the union-shaped fixture no longer matches its purpose: with " +
            s"partialAggregationOptimization=$pushPartialAgg the plan has $partials " +
            s"PartialAggregate nodes:\n$bare")
        }
        spark.sharedState.autoCTECacheManager.clearAll(spark)
        val unionDf = spark.sql(unionCteSql)
        unionDf.collect()
        assert(cachedEntries >= 1,
          s"an aggregate-over-union body must cache with " +
          s"partialAggregationOptimization=$pushPartialAgg; losing it means the structural " +
          s"gate stopped recognising the rewritten aggregate.")
        val unionPlan = unionDf.queryExecution.optimizedPlan
        assert(
          unionPlan.collectWithSubqueries { case r: RepartitionByExpression => r }.isEmpty,
          s"the union-shaped def must not fall through to the repartition shuffle " +
          s"(partialAggregationOptimization=$pushPartialAgg):\n$unionPlan")
      }
    }
  }
  ...
  test("cross-query reuse: two queries with different per-ref filters share cache") {
    prepareData()
    // `keepMergedPredicate` pinned off: this test is about the pre-pushdown body's sharing
    // property. The two queries below filter on DIFFERENT values (col1 1,2 against 3,4), so
    // with the gate on they legitimately cache different bodies -- the implied part that
    // injection puts into the body differs, and the cache key follows it. Sharing under
    // injection is covered where it actually holds, by
    // `AutoCteImpliedInjectionSuite`'s "two queries differing only in an aggregate-output
    // threshold share the injected body".
    withSQLConf(
      SQLConf.AUTO_REUSED_CTE_ENABLED.key -> "true",
      SQLConf.AUTO_CTE_CACHE_KEEP_MERGED_PREDICATE.key -> "false") {
