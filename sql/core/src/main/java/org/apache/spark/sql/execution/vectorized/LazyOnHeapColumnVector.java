...
/**
 * A wrapper around an OnHeapColumnVector that supports lazy materialization.
 * The underlying vector is not populated until the first time a read method is called.
 */
public class LazyOnHeapColumnVector extends OnHeapColumnVector {
  ...
  public OnHeapColumnVector reserveNewColumn(int capacity, DataType type) {
    ...
    OnHeapColumnVector newVector = (OnHeapColumnVector) vector.reserveNewColumn(capacity, type);
    return new LazyOnHeapColumnVector(newVector);
  }
