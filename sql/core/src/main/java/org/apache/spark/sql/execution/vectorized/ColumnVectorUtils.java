...
public class ColumnVectorUtils {
  ...
  public static void populate(WritableColumnVector col, InternalRow row, int fieldIdx) {
    ...
    if (row.isNullAt(fieldIdx)) {
      ...
      if (t == DataTypes.BooleanType) {
        ...
        WritableColumnVector dictionaryIds = col.reserveDictionaryIds(capacity);
        dictionaryIds.putInts(0, capacity, 0);
  ...
  public static void populateOnheap(WritableColumnVector col, InternalRow row, int fieldIdx) {
    int capacity = col.capacity;
    DataType t = col.dataType();
    ...
    if (row.isNullAt(fieldIdx)) {
      ...
      if (t == DataTypes.BooleanType) {
        ...
        col.reserveDictionaryIds(capacity);
        col.setDictionary(ColumnDictionary.of(row.getBinary(fieldIdx)));
      } else if (t == DataTypes.ByteType) {
        col.putBytes(0, capacity, row.getByte(fieldIdx));
      } else if (t == DataTypes.ShortType) {
