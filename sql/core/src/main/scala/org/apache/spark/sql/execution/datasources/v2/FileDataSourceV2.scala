...
trait FileDataSourceV2 extends TableProvider with DataSourceRegister {
  ...
  override def getTable(
    ...
    // Create a fresh table with the provided schema.
    // Don't reuse cached `t` from inferSchema(), which
    // has no userSpecifiedSchema and may fail on empty
    // paths. Fall back to getTable(options) if the
    // provider doesn't support getTable(options, schema).
    val opts = new CaseInsensitiveStringMap(properties)
    val table = try {
      getTable(opts, schema)
    } catch {
      case _: SparkUnsupportedOperationException =>
        if (t != null) t else getTable(opts)
    }
    if (partitioning.nonEmpty) {
      table match {
        case ft: FileTable =>
          // Extract partition column names from
          // IdentityTransform only. BucketTransform
          // and SortedBucketTransform are handled
          // via catalogTable.bucketSpec.
          ft.userSpecifiedPartitioning =
            partitioning.collect {
              case IdentityTransform(
                FieldReference(Seq(col))) => col
            }.toImmutableArraySeq
        case _ =>
      }
    }
