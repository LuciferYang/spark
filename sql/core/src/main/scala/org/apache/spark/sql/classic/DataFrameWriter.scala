...
final class DataFrameWriter[T] private[sql](ds: Dataset[T]) extends sql.DataFrameWriter[T] {
  ...
  private[sql] def saveCommand(path: Option[String]): LogicalPlan = {
    ...
    if (maybeV2Provider.isDefined) {
      ...
      // For FileDataSourceV2 with ErrorIfExists/Ignore, route to the Append/Overwrite branch
      // to handle path existence check (matching V1 semantics), then write via AppendData.
      val fileV2CreateMode = (curmode == SaveMode.ErrorIfExists || curmode == SaveMode.Ignore) &&
        provider.isInstanceOf[FileDataSourceV2]

      curmode match {
        case _ if curmode == SaveMode.Append || curmode == SaveMode.Overwrite || fileV2CreateMode =>
            ...
            case _: TableProvider =>
              try {
                ...
                case _: SparkUnsupportedOperationException =>
                  // Provider doesn't support getTable with schema (e.g., read-only providers).
                  // Fall back to V1.
                  saveToV1SourceCommand(path)
              }
          }
