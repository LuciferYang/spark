...
class DataSourceV2Strategy(session: SparkSession) extends Strategy with PredicateHelper {
  ...
  /**
   * After a V2 file write, discover new partitions on
   * disk and register them in the catalog metastore.
   */
  private def syncNewPartitionsToCatalog(
      ft: FileTable): Unit = {
    ft.catalogTable.foreach { ct =>
      ...
      try {
        ...
        val existing = catalog
          ...
          .map(_.spec).toSet
        // Scan FS for current partitions
        val onDisk = ft.listPartitionIdentifiers(
          Array.empty,
          org.apache.spark.sql.catalyst
            .InternalRow.empty)
        val schema = ft.partitionSchema()
        val tz = session.sessionState.conf
          .sessionLocalTimeZone
        onDisk.foreach { row =>
          val spec = (0 until schema.length).map {
            i =>
          ...
          if (!existing.contains(spec)) {
            val partPath = ft.fileIndex.rootPaths
              .head.suffix("/" + spec.map {
                case (k, v) => s"$k=$v"
              }.mkString("/"))
