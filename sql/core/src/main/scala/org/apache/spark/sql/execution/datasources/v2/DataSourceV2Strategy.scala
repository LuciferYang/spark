...
import org.apache.spark.sql.execution.{FilterExec, InSubqueryExec, LeafExecNode, LocalTableScanExec, ProjectExec, RowDataSourceScanExec, ScalarSubquery, SparkPlan, SparkStrategy => Strategy}
...
class DataSourceV2Strategy(session: SparkSession) extends Strategy with PredicateHelper {
  ...
  override def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
    case PhysicalOperation(project, filters, DataSourceV2ScanRelation(
      v2Relation, V1ScanWrapper(scan, pushed, pushedDownOperators), output, _, _, _)) =>
    ...
    case PhysicalOperation(project, filters,
        DataSourceV2ScanRelation(_, scan: LocalScan, output, _, _, _)) =>
    ...
    case DeleteFromTable(relation, condition) =>
      relation match {
        case DataSourceV2ScanRelation(r, _, output, _, _, _) =>
          val table = r.table
...
private[sql] object DataSourceV2Strategy extends Logging {
  ...
  protected[sql] def translateRuntimeFilterV2(expr: Expression): Option[Predicate] = expr match {
    ...
    case expr: Expression if expr.exists(_.isInstanceOf[ScalarSubquery]) =>
