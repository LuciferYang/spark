 ...
 * satisfied.
 *
 * Wired directly into the plan-preparation pipelines (`QueryExecution.preparations`
 * and AQE's `queryStagePreparationRules`), after `EnsureRequirements` and before
 * `RemoveRedundantSorts`. The rule never adds or removes
