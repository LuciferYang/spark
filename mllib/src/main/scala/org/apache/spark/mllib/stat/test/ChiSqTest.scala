...
private[spark] object ChiSqTest extends Logging {
  ...
  private def computeChiSq(
    ...
    val label2Index = Utils.toIndexMap(counts.iterator.map(_._1._1).toArray.distinct.sorted)
    ...
    val value2Index = Utils.toIndexMap(counts.iterator.map(_._1._2).toArray.distinct.sorted)
