...
public abstract class AbstractBytesToBytesMapSuite {
  ...
  public void emptyMap() {
    ...
    try {
      ...
      assertFalse(map.lookup(key, Platform.BYTE_ARRAY_OFFSET, keyLengthInBytes).isDefined());
      assertFalse(map.iterator().hasNext());
      assertFalse(map.iteratorWithKeyIndex().hasNext());
    } finally {
      map.free();
    }
  }
  ...
  private void iteratorTestBase(boolean destructive, boolean isWithKeyIndex) throws Exception {
    ...
    assertEquals(size / 2, map.maxNumKeysIndex());
    try {
      ...
      while (iter.hasNext()) {
        ...
        if (keyLength != 0 && isWithKeyIndex) {
          ...
          assertTrue(expectedLoc.isDefined() &&
            expectedLoc.getKeyIndex() == loc.getKeyIndex());
        }
      }
      if (destructive) {
        // Latest page is not freed by iterator but by map itself
        assertEquals(countFreedPages, numPages - 1);
      }
      assertEquals(size, valuesSeen.cardinality());
    } finally {
      map.free();
    }
  }
  ...
  public void iteratingOverDataPagesWithWastedSpace() throws Exception {
    ...
    try {
      ...
      assertEquals(NUM_ENTRIES, valuesSeen.cardinality());
    } finally {
      map.free();
    }
  }
    ...
    try {
      ...
      for (int i = 0; i < size * 0.9; i++) {
        ...
        if (!expected.containsKey(ByteBuffer.wrap(key))) {
          ...
          assertTrue(arrayEquals(key, loc.getKeyBase(), loc.getKeyOffset(), key.length));
          assertTrue(
  ...
  public void spillInIterator() throws IOException {
    ...
    try {
      ...
      assertTrue(iter2.spill(1024) >= 1024);
      for (i = 100; i < 1024; i++) {
        iter2.next();
      }
