...
object PlatformBenchmark extends BenchmarkBase {
  ...
  override def runBenchmarkSuite(mainArgs: Array[String]): Unit = {
    ...
    val counts = Seq((count4k, str4k), (count1m, str1m), (count4m, str4m), (count8m, str8m))
      ...
      runBenchmark(s"Platform Bulk Operations $countStr Ints") {
        ...
        benchmark.addCase("manual: Heap -> Heap") { _ =>
          ...
          while (i < 10) {
            var j = 0
            while (j < count) {
              dstArray(j) = Platform.getInt(srcArray, Platform.INT_ARRAY_OFFSET + j * 4)
              j += 1
            }
            i += 1
          }
        ...
        benchmark.addCase("manual: Off-heap -> Heap") { _ =>
          ...
          while (i < 10) {
            var j = 0
            while (j < count) {
              dstArray(j) = Platform.getInt(null, address + j * 4)
              j += 1
            }
            i += 1
          }
