    ...
    new RDDStorageInfo(
      ...
      dataDistribution =
        if (info.getDataDistributionList.isEmpty) {
          ...
          Option(Utils.javaListToSeq(info.getDataDistributionList)(deserializeRDDDataDistribution))
        },
      partitions =
        Option(Utils.javaListToSeq(info.getPartitionsList)(deserializeRDDPartitionInfo))
    )
  }
