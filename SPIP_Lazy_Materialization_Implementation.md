...
### 2.1 新增类：`LazyOnHeapColumnVector` 和 `LazyOffHeapColumnVector`
**位置**:
- `sql/core/src/main/java/org/apache/spark/sql/execution/vectorized/LazyOnHeapColumnVector.java`
- `sql/core/src/main/java/org/apache/spark/sql/execution/vectorized/LazyOffHeapColumnVector.java`

这两个类分别是 `OnHeapColumnVector` 和 `OffHeapColumnVector` 的装饰器（Wrapper），实现了延迟加载的核心逻辑。
...
2.  **向量分配**: 在 `allocateColumns` 方法中，如果启用了延迟物化，将分配的 `OnHeap` 或 `OffHeap` 向量分别包装在 `LazyOnHeapColumnVector` 或 `LazyOffHeapColumnVector` 中。
3.  **批次读取逻辑 (`nextBatch`)**:
    *   在处理每一列时，检查是否为 `Lazy...ColumnVector` 且启用了延迟物化。
...
*   验证了 `LazyOnHeapColumnVector` 在 `put` 操作时**不会**触发加载（保持 Lazy）。
