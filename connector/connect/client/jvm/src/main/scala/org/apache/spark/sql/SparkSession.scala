...
class SparkSession private[sql] (
  ...
  def sql(sqlText: String, args: Array[_]): DataFrame = newDataFrame { builder =>
    ...
    builder.mergeFrom(executeSqlCommand(sqlCommand))
  }
  ...
  private def executeSqlCommand(sqlCommand: proto.SqlCommand): proto.Relation = {
    ...
    try {
      ...
      response.getSqlCommandResult.getRelation
    } finally {
