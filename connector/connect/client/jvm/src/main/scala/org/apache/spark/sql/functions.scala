...
// scalastyle:on
  ...
  private def createLambda(f: Column => Column): Column = Column { builder =>
    val x = UnresolvedNamedLambdaVariable.freshVarName("x")
    builder.getLambdaFunctionBuilder
      .setFunction(f(Column(x)).expr)
      .addArguments(proto.Expression.UnresolvedNamedLambdaVariable.newBuilder().addNameParts(x))
  }

  private def createLambda(f: (Column, Column) => Column): Column =
    Column { builder =>
      val x = UnresolvedNamedLambdaVariable.freshVarName("x")
      val y = UnresolvedNamedLambdaVariable.freshVarName("y")
      builder.getLambdaFunctionBuilder
        .setFunction(f(Column(x), Column(y)).expr)
        .addArguments(proto.Expression.UnresolvedNamedLambdaVariable.newBuilder().addNameParts(x))
        .addArguments(proto.Expression.UnresolvedNamedLambdaVariable.newBuilder().addNameParts(y))
    }
  ...
  private def createLambda(f: (Column, Column, Column) => Column): Column =
    Column { builder =>
      val x = UnresolvedNamedLambdaVariable.freshVarName("x")
      val y = UnresolvedNamedLambdaVariable.freshVarName("y")
      val z = UnresolvedNamedLambdaVariable.freshVarName("z")
      builder.getLambdaFunctionBuilder
        .setFunction(f(Column(x), Column(y), Column(z)).expr)
        .addArguments(proto.Expression.UnresolvedNamedLambdaVariable.newBuilder().addNameParts(x))
        .addArguments(proto.Expression.UnresolvedNamedLambdaVariable.newBuilder().addNameParts(y))
        .addArguments(proto.Expression.UnresolvedNamedLambdaVariable.newBuilder().addNameParts(z))
    }
