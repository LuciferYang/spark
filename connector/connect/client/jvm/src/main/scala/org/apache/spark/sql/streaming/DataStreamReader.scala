...
final class DataStreamReader private[sql] (sparkSession: SparkSession) extends Logging {
  ...
  private def loadInternal(path: Option[String]): DataFrame = {
    sparkSession.newDataFrame { builder =>
      val dataSourceBuilder = builder.getReadBuilder.setIsStreaming(true).getDataSourceBuilder
      ...
      path.foreach(path => dataSourceBuilder.addPaths(path))
      builder.build()
    }
  }
  ...
  def table(tableName: String): DataFrame = {
    sparkSession.newDataFrame { builder =>
      val readBuilder = builder.getReadBuilder
      readBuilder
        .setIsStreaming(true)
